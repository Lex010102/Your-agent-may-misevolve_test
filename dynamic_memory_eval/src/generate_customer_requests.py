...
def main() -> None:
    ...
    while next_id <= args.n:
        ...
        raw = client.generate(model=cfg.model_customer_agent, user_prompt=prompt, temperature=1.0, max_completion_tokens=2048)
        ...
        for expected_id, item_any in zip(expected_ids, reqs, strict=True):
            ...
            if item.get("id") != expected_id:
                raise RuntimeError(f"Bad id sequence: expected {expected_id}, got {item.get('id')!r}")
            ...
            seen.add(key)
            out.append({"id": expected_id, "request": text})
